    ...
    $.fn.directionalHover = function(options) {
        ...
        function findSide(flags, overlay, direction, x, y, w, h) {
            if (testMask(flags, tlMask)) {
                testTopLeftToBottomRight(x, y, w, h) ? setOverlayPosition(overlay, direction, 0, -w*3) : setOverlayPosition(overlay, direction, -h*3, 0);
            }
            else if (testMask(flags, trMask)) {
                testBottomRightToTopLeft(x, y, w, h) ? setOverlayPosition(overlay, direction, -h*3, 0) : setOverlayPosition(overlay, direction, 0, w*3);
            }
            else if (testMask(flags, blMask)) {
                testBottomRightToTopLeft(x, y, w, h) ? setOverlayPosition(overlay, direction, 0, -w*3) : setOverlayPosition(overlay, direction, h*3, 0);
            }
            else if (testMask(flags, brMask)) {
                testTopLeftToBottomRight(x, y, w, h) ? setOverlayPosition(overlay, direction, h*3, 0) : setOverlayPosition(overlay, direction, 0, w*3);
            }
        }
